...
class ClassificationEvaluator(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        logits_4x4, logits_8x8, logits_16x16 = self.rand_ms_forward(x)

        loss_4x4 = self.loss_fn(logits_4x4, y)
        acc_4x4 = self.acc_0(logits_4x4, y)
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(name='ft-part-conv-uniViT-add-random-resize-3conv-fix14token',
                               project='uniViT',
                               entity='pigpeppa', offline=False)
    checkpoint_callback = ModelCheckpoint(monitor="val_acc_16x16", mode="max",
                                          dirpath='ckpt/uniViT/add_random_resize_3conv_fix14token',
                                          save_top_k=1,
                                          save_last=True)
