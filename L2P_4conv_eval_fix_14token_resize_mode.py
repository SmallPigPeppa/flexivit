...
class ClassificationEvaluator(pl.LightningModule):
    ...
    def ms_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_0 = self.patch_embed_4x4(x, patch_size=self.patch_size, mode=self.mode)

        # x_1 = F.interpolate(x, size=112, mode='bilinear')
        x_1 = self.patch_embed_8x8(x, patch_size=self.patch_size, mode=self.mode)

        # x_2 = F.interpolate(x, size=168, mode='bilinear')
        x_2 = self.patch_embed_12x12(x, patch_size=self.patch_size, mode=self.mode)

        # x_3 = F.interpolate(x, size=224, mode='bilinear')
        x_3 = self.patch_embed_16x16(x, patch_size=self.patch_size, mode=self.mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default='area')
