...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(name='ft-part-conv-uniViT-add-random-resize-4conv-fix14token-debug',
                               project='uniViT',
                               entity='pigpeppa', offline=False)
    checkpoint_callback = ModelCheckpoint(monitor="val_acc_16x16", mode="max",
                                          dirpath='ckpt/uniViT/add_random_resize_4conv_fix14token_debug',
                                          save_top_k=1,
                                          save_last=True)
