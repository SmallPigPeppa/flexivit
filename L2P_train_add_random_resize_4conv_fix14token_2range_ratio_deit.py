...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(name='add-random-resize-4conv-fix14token-2range-ratio-deit', project='L2P',
                               entity='pigpeppa', offline=False)
    checkpoint_callback = ModelCheckpoint(monitor="val_acc_16x16", mode="max",
                                          dirpath='ckpt/L2P/add_random_resize_4conv_fix14token_2range_ratio/deit',
                                          save_top_k=1,
                                          save_last=True)
