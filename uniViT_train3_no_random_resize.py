...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    args["logger"] = False  # Disable saving logging artifacts
    wandb_logger = WandbLogger(name='ft-part-conv-uniViT-no-random-resize', project='uniViT', entity='pigpeppa', offline=False)
    checkpoint_callback = ModelCheckpoint(monitor="val_acc_2", mode="max", dirpath='ckpt/uniViT/no_random_resize', save_top_k=1,save_last=True)
